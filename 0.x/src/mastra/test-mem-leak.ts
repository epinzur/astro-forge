...
const filePath = "/Users/epinzur/src/github.com/epinzur/astro-forge/shared/memory_leak.png";
...
async function runTest() {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    console.log(`[mem-leak-test] DEBUG: Heap Before Workflow: ${formatMB(heapBefore)}`);
    ...
    console.log(`[mem-leak-test] DEBUG: Heap After Workflow: ${formatMB(heapAfter)}`);
    console.log(`[mem-leak-test] DEBUG: Heap Delta: ${formatMB(heapAfter - heapBefore)}`);
    console.log(`[mem-leak-test] DEBUG: Iteration Time: ${formatMs(iterDuration)}`);

    if (result.status === "success") {
      const responseLength = (result.result?.response?.length ?? 0) / 1024 / 1024;
      console.log(`[mem-leak-test] DEBUG: Response text length: ${responseLength.toFixed(4)} MB`);
    } else {
      console.log(`[mem-leak-test] DEBUG: Workflow status: ${result.status}`);
      console.log(`[mem-leak-test] DEBUG: Result:`, JSON.stringify(result, null, 2));
    }
  }
